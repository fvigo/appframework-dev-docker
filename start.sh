...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the developer username if it exists
    if id developer &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER developer
    fi
    ...
    if [[ "$NB_USER" != "developer" ]]; then
        # changing username, make sure homedir exists
        # (it could be mounted, and we shouldn't create it if it already exists)
        if [[ ! -e "/home/$NB_USER" ]]; then
            echo "Relocating home dir to /home/$NB_USER"
            mv /home/developer "/home/$NB_USER"
        fi
        # if workdir is in /home/developer, cd to /home/$NB_USER
    ...
    if [[ "$NB_UID" == "$(id -u developer)" && "$NB_GID" == "$(id -g developer)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^developer:/developer:/" > /tmp/passwd
                echo "developer:x:$(id -u):$(id -g):,,,:/home/developer:/bin/bash" >> /tmp/passwd
